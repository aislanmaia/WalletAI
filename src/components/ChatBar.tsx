  ...
  return (
    <div className="fixed left-0 right-0 bottom-0 z-30 md:ml-[var(--sidebar-width)] px-4 pb-4 pt-2 bg-transparent">
      ...
      {!isExpanded && suggestions.length > 0 && (
        <div className="max-w-3xl xl:max-w-4xl 2xl:max-w-5xl mx-auto flex flex-wrap gap-2 mb-2">
          {suggestions.map((s) => (
            <button
              key={s}
        ...
        >
            ...
            {isExpanded && (
              <div className="h-full flex flex-col">
                ...
                <div className="flex-1 min-h-0 flex flex-row">
                  {/* Coluna mensagens */}
                  <div className="flex-[2] min-w-0 flex flex-col border-r border-gray-100 dark:border-white/10 relative">
                  <div className="flex-1 min-h-0 overflow-y-auto bg-transparent pr-2 overscroll-contain" onScroll={onMessagesScroll}>
                    ...
                    {showScrollToBottom && (
                      <button
                        className="absolute right-4 bottom-8 inline-flex items-center gap-1 rounded-full bg-white/90 backdrop-blur px-2 py-1 text-xs text-gray-700 ring-1 ring-gray-200 shadow-sm hover:bg-white dark:bg-neutral-800/80 dark:text-gray-200 dark:ring-white/10"
                    ...
                    <button
                      className="absolute right-4 bottom-2 inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs bg-white ring-1 ring-gray-200 shadow-sm hover:bg-gray-50 text-gray-600 hover:text-gray-800 dark:bg-neutral-900/70 dark:text-gray-200 dark:ring-white/10"
                      onClick={() => {
                  ...
                  <aside className="flex-[1] min-w-[155px] max-w-[175px] hidden md:flex flex-col bg-white dark:bg-neutral-900">
          ...
          <div className="px-2 py-1.5">
            <div className="flex items-center gap-2 rounded-2xl border border-gray-200 supports-[backdrop-filter]:bg-white/90 backdrop-blur px-2 dark:border-white/10 dark:bg-neutral-900/70 transition-all duration-300">
              ...
              <Button onClick={handleSubmit} disabled={!value.trim() || isProcessing} className="h-8 w-8 rounded-full bg-blue-500 text-white hover:bg-blue-600" aria-label="Enviar mensagem">
